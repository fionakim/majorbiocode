...
class FastqFile(File):
        ...
        if self.is_gz:
            ...
            with open(self.prop['path'], 'rb') as f:
                ...
                for line in f:
                    ...
                    if len(line2) != len(line4):
                        raise Exception("第{}行碱基的长度与它的质量文件的长度不相等, 碱基长度为{}, 质量长度为{}".format(str(count), str(len(line2)),
                                                                                          str(len(line4))))
                    count = count + 4
            self.set_property("samples", self.samples)
        return self.samples
    ...
    def get_seq_length_mode(self):
        '''
        为了找到fq文件中读长的长长度最集中的数值
        :return: reads长度总体中的众数(整型，且如果得到多个众数，只取一个返回)
        '''
        cmd = "cat %s | paste - - - - | awk -F '\t' '{print length($2)}' | sort | uniq -c | sort -k 1,1nr " % self.prop['path']
        try:
            count_length = subprocess.check_output(cmd, shell=True)
            max_count_length = int(re.split('\W', count_length.strip())[1])
        except subprocess.CalledProcessError:
            raise Exception('获取fastq文件reads长度分布出错')
