...
class MapspliceModule(Module):
        ...
        if self.option('have_index') == 0:
            ...
            if mapsplice_fa_dir_obj.check():
                ...
                if len(self.index_tools) == 1:
                    self.index_tools[0].on('end', self.set_index_output())

                else:
                    self.on_rely(self.index_tools, self.set_index_output())
                    self.on_rely(self.index_tools, self.finish_update)
    ...
    def set_index_output(self):
        ...
        self.linkdir(single_fa_dir, os.path.basename(self.ref_single_seq_fa))
        if self.option('have_index') == 1:
            index_dir = self.option('single_fa_index_dir').path
            self.linkdir(index_dir, os.path.basename(self.ref_seq_fa_index))
        if self.option('have_index') == 0:
            for tool in self.index_tools:
                self.linkdir(tool.output_dir, os.path.basename(self.ref_seq_fa_index))
    ...
    def mapsplice_map_run(self):
        ...
        map_options = {"fa_contain_dir": self.ref_single_seq_fa,
        ...
        for sample in sample_list_dic.keys():
            ...
            if self.option('seq_type') == 'paired':
                if sample_list_dic[sample].has_key('r') and sample_list_dic[sample].has_key('l'):
                    map_options["_1_fq"] = os.path.join(self.option("fastq_dir").path, sample_list_dic[sample]['r'])
                    map_options["_2_fq"] = os.path.join(self.option("fastq_dir").path, sample_list_dic[sample]['l'])
                else:
            ...
            mapsplice_map.set_option(map_options)
    ...
    def set_map_output(self):
        ...
        for map_tool in self.mapsplice_map_tools:
            self.linkdir(map_tool.output_dir, os.path.basename(map_tool.work_dir))
